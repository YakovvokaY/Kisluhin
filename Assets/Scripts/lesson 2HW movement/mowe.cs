...
public class mowe : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (transform.rotation.x*360 > 45)
        {
            transform.Rotate(new Vector3(1, 0, 0) * Time.deltaTime * 400);
        }
        if (transform.rotation.x * 360 < -45)
        {
            transform.Rotate(new Vector3(-1, 0, 0) * Time.deltaTime * 400);
        ...
        if (transform.rotation.z * 360 > 45)
        {
            transform.Rotate(new Vector3(0, 0, 1) * Time.deltaTime * 400);
        }
        if (transform.rotation.z * 360 < -45)
        {
            transform.Rotate(new Vector3(0, 0, -1) * Time.deltaTime * 400);
